...
export const actions = {
    ...
    updateListItem: (item, text, dateString) => ({ type: types.UPDATE_LIST_ITEM, payload: { item, text, dateString } }),
...
function updateListItem(state, action) {
    ...
    const updatedLists = state.lists.map(itemList =>
        itemList.updateItem(item, item.updateWith({ text, date_string: dateString }))
    );
    return { ...state, lists: updatedLists };
}
